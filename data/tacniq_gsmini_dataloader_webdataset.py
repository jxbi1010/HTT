#!/usr/bin/env python3
"""
Tacniq Gsmini Dataset Dataloader using WebDataset for TAR format.
...
def tacniq_collate_fn(batch):
    """Custom collate function for tacniq_gsmini multimodal data."""
    images_data = []
    tactile_data = []
    object_idx_data = []
...
class TacniqGsminiDatasetWebDataset(IterableDataset):
    """Dataset for Tacniq Gsmini using WebDataset for safe tar file loading."""
    
    # Each tacniq chunk in the tar file contains 10 frames (tacniq is 10x frequency of images)
    TACNIQ_FRAMES_PER_CHUNK = 10
    ...
    def __init__(self,
        ...
        """
        Initialize Tacniq Gsmini Dataset using WebDataset.
        ...
        Args:
            ...
            apply_background_subtraction: Whether to subtract background. If None, read from config/sensor/gsmini_config.yaml
            normalize_images: Whether to normalize image data. If None, read from config/sensor/gsmini_config.yaml
            normalize_tactile: Whether to normalize tactile data. If None, read from config/sensor/gsmini_config.yaml
        ...
        try:
            ...
            data_root = resolve_tar_dir(str(data_root), dataset_split_type, "tacniq_gsmini")
    ...
    def _load_normalization_stats(self):
        ...
        if self.normalize_images:
            sensor_config_path = config_dir / 'gsmini.yaml'
        ...
        if self.normalize_tactile:
            sensor_config_path = config_dir / 'tacniq.yaml'
            if sensor_config_path.exists():
                with open(sensor_config_path, 'r') as f:
                    config = yaml.safe_load(f) or {}
                    tactile_mean = config.get('tactile_mean', [0.0] * 66)  # Tacniq has 66 features
                    tactile_std = config.get('tactile_std', [1.0] * 66)
    ...
    def _load_reference_background(self):
        ...
        if "images" in self.load_modalities or self.apply_background_subtraction:
            bg_image_path = bg_data_dir / 'gsmini.png'
            if bg_image_path.exists():
                bg_image = Image.open(bg_image_path)
                bg_array = np.array(bg_image, dtype=np.uint8)  # PIL loads as RGB [H, W, C]
                # Background is already in RGB format, no conversion needed
                # Keep as [H, W, C] format to match image format before permute
                # Will be converted to [C, H, W] when used in _process_sequence
                self.reference_bg = torch.from_numpy(bg_array).float() / 255.0  # [H, W, C] in RGB
            else:
                self.reference_bg = None
        
        # Tactile background
        if "tactile" in self.load_modalities or self.apply_background_subtraction:
            bg_tactile_path = bg_data_dir / 'tacniq.npy'
    ...
    def _compute_episode_to_object_mapping(self):
        ...
        # Get list of tar files (sharded or single)
        # tar_files should be set by _find_tar_files() which is called before this
        if not hasattr(self, 'tar_files'):
            # Fallback: try to get from tar_file attribute
            if hasattr(self, 'tar_file') and self.tar_file:
                tar_files = [self.tar_file]
            else:
                print("Warning: No tar_files or tar_file found. Cannot create episode-to-object mapping.")
                return
        else:
    ...
    def _compute_total_length(self):
        ...
        Reads manifests from all shards (if multiple) or single tar file and calculates total chunks based on chunk sizes.
        ...
        total_chunks = 0
        # Ensure tar_files is available
        if not hasattr(self, 'tar_files') or not self.tar_files:
            if not hasattr(self, 'tar_file') or not self.tar_file:
                # Tar files not found yet, return a default estimate
                print(f"Warning: Tar files not found when computing length. Returning default estimate.")
        ...
        try:
            import tarfile
            # Read from all shards to get complete count
            for tar_file_path in tar_files:
                try:
                    # Check if file exists
                    if not Path(tar_file_path).exists():
                        print(f"Warning: Tar file not found: {tar_file_path}")
                        continue
                    ...
                    with tarfile.open(tar_file_path, 'r') as tar:
                        ...
                        for member in tar.getmembers():
                            if member.name == '_manifest.json':
                                ...
                                for episode_meta in manifest.get('episodes', []):
                                    ...
                                    num_tacniq_chunks = episode_meta.get('num_tacniq_chunks', 0)
                                    
                                    # For tacniq, chunks are 1:1 aligned with images
                                    # Each image has one tacniq chunk (which contains TACNIQ_FRAMES_PER_CHUNK=10 frames)
                                    # This ensures tacniq data length is 10x gsmini data length:
                                    #   - Total tacniq frames = num_tacniq_chunks * TACNIQ_FRAMES_PER_CHUNK
                                    #   - Total gsmini frames = num_images
                                    #   - Ratio: (num_tacniq_chunks * 10) / num_images = 10 (when num_tacniq_chunks == num_images)
                                    # Validate the 1:1 alignment to ensure 10:1 ratio
                                    if num_images > 0 and num_tacniq_chunks > 0:
                                        if num_tacniq_chunks != num_images:
                                            print(f"Warning: Episode {episode_meta.get('episode_name', 'unknown')} has "
                                                  f"num_images={num_images} but num_tacniq_chunks={num_tacniq_chunks}. "
                                                  f"Expected 1:1 alignment (num_tacniq_chunks == num_images) to ensure "
                                                  f"tacniq data length is 10x gsmini data length. "
                                                  f"Current ratio: {num_tacniq_chunks * self.TACNIQ_FRAMES_PER_CHUNK / num_images:.2f}x")
                                    # So we can create chunks based on image_chunk_size
                                    ...
                                    # For tacniq, each chunk in tar file has TACNIQ_FRAMES_PER_CHUNK (10) frames
                                    # We need to calculate how many chunks we can create from the available chunks
                                    if "tactile" in self.load_modalities:
                                        # Calculate how many raw chunks we need to get tactile_chunk_size frames
                                        num_chunks_needed_per_sample = (self.tactile_chunk_size + self.TACNIQ_FRAMES_PER_CHUNK - 1) // self.TACNIQ_FRAMES_PER_CHUNK
                                        # Each episode has num_tacniq_chunks raw chunks, each with 10 frames
                                        # We can create this many samples: num_tacniq_chunks // num_chunks_needed_per_sample
                                        tactile_chunks = num_tacniq_chunks // num_chunks_needed_per_sample if num_tacniq_chunks >= num_chunks_needed_per_sample else 0
                                    else:
                        ...
                        if not manifest_found:
                            print(f"Warning: No manifest found in {Path(tar_file_path).name}")
                except Exception as e:
                    # Continue to next shard if one fails
        ...
        # If total_chunks is 0, it might indicate a problem, but don't fail
        # The dataset might still work if samples are being fetched
        if total_chunks == 0:
            print(f"Warning: Computed total length is 0. This might indicate an issue with the dataset or chunk size configuration.")
            print(f"  Image chunk size: {self.image_chunk_size}")
            print(f"  Tactile chunk size: {self.tactile_chunk_size}")
            print(f"  Load modalities: {self.load_modalities}")
            print(f"  Tar files: {[Path(f).name for f in tar_files]}")
            # Don't set to 0 - use a default estimate instead to avoid the warning
            # But log that we're using an estimate
            self._total_length = 1000000
            return self._total_length
    ...
    def _decode_image(self, data):
        ...
        NOTE: For gsmini, the stored JPEG channel order is effectively swapped; to get
    ...
    def _extract_episode_and_index(self, filename):
        ...
        Pattern: episode_XXX_YYYYY.gsmini.jpg or episode_XXX_YYYYY.tacniq.npy
        e.g., episode_1_000000.gsmini.jpg -> (episode_1, 0)
        """
        # Pattern: episode_XXX_YYYYY.gsmini.jpg or episode_XXX_YYYYY.tacniq.npy
        parts = filename.split('_')
        if len(parts) >= 3:
            episode_name = f"{parts[0]}_{parts[1]}"
            # Index is before the first dot (e.g., "000000.gsmini.jpg" -> "000000")
    ...
    def _process_sample(self, sample):
        ...
        contains both the .gsmini.jpg and .tacniq.npy files for the same timestep.
        
        Note: WebDataset uses keys like 'gsmini.jpg' and 'tacniq.npy' (not full filenames).
        ...
        # e.g., __key__ = "episode_951_000000" for files episode_951_000000.gsmini.jpg and episode_951_000000.tacniq.npy
        ...
        if "images" in self.load_modalities:
            # WebDataset uses key 'gsmini.jpg' (not 'episode_XXX_YYYYY.gsmini.jpg')
            # Try both 'gsmini.jpg' and variations
            for key in ['gsmini.jpg', 'gsmini.png']:
                if key in sample:
                    image = self._decode_image(sample[key])
        ...
        if "tactile" in self.load_modalities:
            # WebDataset uses key 'tacniq.npy' or 'tac02.npy' (not 'episode_XXX_YYYYY.tacniq.npy')
            for key in ['tacniq.npy', 'tac02.npy']:
                if key in sample:
                    tactile_chunk = self._decode_npy(sample[key])
    ...
    def _process_sequence(self, image_sequence, tactile_sequence, object_name):
        ...
        if "images" in self.load_modalities and image_sequence:
            ...
            # Scale from [0, 255] to [0, 1] range (assume data not normalized)
            ...
            if self.apply_background_subtraction and self.reference_bg is not None:
                ...
                # Take absolute value after subtraction
            ...
            # Normalize (complete normalization: subtract mean, divide by std)
            if self.normalize_images:
                if self.image_mean is not None:
                    images_tensor = images_tensor - self.image_mean
                if self.image_std is not None:
                    images_tensor = images_tensor / (self.image_std + 1e-6)
                # Note: No clamping for images (matching 9dtact behavior)
            
            # Convert to CHW
            # Images are now in correct RGB format [T, H, W, C] where C is RGB
            images_tensor = images_tensor.permute(0, 3, 1, 2)  # [T, C, H, W] with C in RGB order
            result['images'] = images_tensor
        
        # Process tactile: concatenate consecutive chunks
        # Each chunk from the tar file contains TACNIQ_FRAMES_PER_CHUNK (10) frames
        ...
        if "tactile" in self.load_modalities and tactile_sequence:
            ...
            num_chunks_needed = (self.tactile_chunk_size + self.TACNIQ_FRAMES_PER_CHUNK - 1) // self.TACNIQ_FRAMES_PER_CHUNK  # Ceiling division
            
            # Take only the needed number of chunks
            chunks_to_use = tactile_sequence[:num_chunks_needed]
            
            if chunks_to_use:
                # Each chunk is [10, D] where D is tactile dimension
                # Concatenate along time dimension to get [tactile_chunk_size, D]
                tactile_chunk = np.concatenate(chunks_to_use, axis=0)  # [total_frames, D]
            
                # Ensure we have exactly tactile_chunk_size frames
                if tactile_chunk.shape[0] > self.tactile_chunk_size:
                    tactile_chunk = tactile_chunk[:self.tactile_chunk_size]
                elif tactile_chunk.shape[0] < self.tactile_chunk_size:
                    # Pad with last frame if needed
                    padding_needed = self.tactile_chunk_size - tactile_chunk.shape[0]
                    last_frame = tactile_chunk[-1:]
                    padding = np.tile(last_frame, (padding_needed, 1))
                    tactile_chunk = np.concatenate([tactile_chunk, padding], axis=0)
            else:
                # Fallback: create empty tactile tensor
                tactile_chunk = np.zeros((self.tactile_chunk_size, 66), dtype=np.float32)  # Default tacniq dim is 66
            ...
            # Normalize (complete normalization: subtract mean, divide by std)
            if self.normalize_tactile:
                if self._tactile_mean_tensor is not None:
                    tactile_tensor = tactile_tensor - self._tactile_mean_tensor.unsqueeze(0)
                if self._tactile_std_tensor is not None:
                    tactile_tensor = tactile_tensor / (self._tactile_std_tensor.unsqueeze(0) + 1e-6)
    ...
    def __iter__(self):
        ...
        # Calculate how many tactile chunks we need (each chunk has TACNIQ_FRAMES_PER_CHUNK frames)
        num_tactile_chunks_needed = (self.tactile_chunk_size + self.TACNIQ_FRAMES_PER_CHUNK - 1) // self.TACNIQ_FRAMES_PER_CHUNK if "tactile" in self.load_modalities else 0
        
        # Sequence buffers using deque
        image_seq = deque(maxlen=self.image_chunk_size)
        tactile_seq = deque(maxlen=num_tactile_chunks_needed)  # Store chunks, not individual frames
        ...
        for sample_data in dataset:
            ...
            # If object_name is still 'unknown', try to get it from episode_to_object mapping
            if obj_name == 'unknown' and episode_name and hasattr(self, 'episode_to_object'):
                obj_name = self.episode_to_object.get(episode_name, 'unknown')
            ...
            if is_new_episode or is_non_consecutive:
                ...
                if len(image_seq) > 0 or len(tactile_seq) > 0:
                    ...
                    # For tactile, take only the needed number of chunks
                    ...
                    if seq_images or seq_tactile:
                        # Ensure object_name is set before processing sequence
                        if object_name == 'unknown' and current_episode and hasattr(self, 'episode_to_object'):
                            object_name = self.episode_to_object.get(current_episode, 'unknown')
                ...
                # If object_name is still 'unknown', look it up from episode_to_object mapping
                if object_name == 'unknown' and episode_name and hasattr(self, 'episode_to_object'):
                    object_name = self.episode_to_object.get(episode_name, 'unknown')
            else:
            ...
            # For tactile: check if we have enough chunks (each chunk has TACNIQ_FRAMES_PER_CHUNK frames)
            ...
            if has_enough_images and has_enough_tactile:
                ...
                # Update object_name from current sample if it's still unknown
                if object_name == 'unknown' and obj_name != 'unknown':
                    object_name = obj_name
                # Also try to get from episode_to_object mapping if still unknown
                if object_name == 'unknown' and episode_name and hasattr(self, 'episode_to_object'):
                    object_name = self.episode_to_object.get(episode_name, 'unknown')
        ...
        if len(image_seq) > 0 or len(tactile_seq) > 0:
            ...
            seq_tactile = list(tactile_seq)[:num_tactile_chunks_needed] if len(tactile_seq) >= num_tactile_chunks_needed else list(tactile_seq)
            if seq_images or seq_tactile:
                # Ensure object_name is set (use the last known object_name from the sequence)
                if object_name == 'unknown' and current_episode and hasattr(self, 'episode_to_object'):
                    object_name = self.episode_to_object.get(current_episode, 'unknown')
                result = self._process_sequence(seq_images, seq_tactile, object_name)
                if result:
...
def create_tacniq_dataloaders(data_root: str,
                              batch_size: int = 32,
                              image_chunk_size: int = 5,
                              tactile_chunk_size: int = 50,
                              num_workers: int = 4,
    ...
    gsmini images and tacniq tactile data interleaved. Files are named with matching
    ...
    train_dataset = TacniqGsminiDatasetWebDataset(
        data_root=data_root,
        split="train",
    ...
    val_dataset = TacniqGsminiDatasetWebDataset(
        data_root=data_root,
        split="val",
        image_chunk_size=image_chunk_size,
        tactile_chunk_size=tactile_chunk_size,
    ...
    test_dataset = TacniqGsminiDatasetWebDataset(
        data_root=data_root,
        split="test",
        image_chunk_size=image_chunk_size,
        tactile_chunk_size=tactile_chunk_size,
...
def test_dataloader(data_root: str = 'data/tacniq_gsmini_tar',
                    batch_size: int = 4,
                    dataset_split_type: str = 'pretrain',
    ...
    print("=" * 60)
    print("Testing Tacniq Gsmini WebDataset Dataloader")
    print("=" * 60)
    print(f"\nConfiguration:")
...
def _load_config_settings(config_path: Optional[str] = None) -> Dict[str, bool]:
    ...
    Args:
        config_path: Path to config file. If None, tries to find gsmini_config.yaml in config/sensor/
    
    Returns:
        Dict with 'apply_background_subtraction', 'normalize_images', 'normalize_tactile'
    """
    if config_path is None:
        config_path = Path(__file__).parent.parent / 'config' / 'sensor' / 'gsmini_config.yaml'
    else:
    ...
    try:
        ...
        if 'data' in config:
            ...
            defaults['apply_background_subtraction'] = data_config.get('apply_background_subtraction', defaults['apply_background_subtraction'])
            defaults['normalize_images'] = data_config.get('normalize_images', defaults['normalize_images'])
            defaults['normalize_tactile'] = data_config.get('normalize_tactile', defaults['normalize_tactile'])
        else:
            # Check at root level
            defaults['apply_background_subtraction'] = config.get('apply_background_subtraction', defaults['apply_background_subtraction'])
            defaults['normalize_images'] = config.get('normalize_images', defaults['normalize_images'])
            defaults['normalize_tactile'] = config.get('normalize_tactile', defaults['normalize_tactile'])
    except Exception as e:
...
def visualize_samples(data_root: str = 'data/tacniq_gsmini_tar',
                      batch_size: int = 4,
                      dataset_split_type: str = 'pretrain',
                      load_modalities: List[str] = ['images', 'tactile'],
                      num_workers: int = 0,
                      num_samples: int = 4,
                      output_path: str = 'data/tacniq_sample_visualization.png',
                      normalize_images: Optional[bool] = None,
                      normalize_tactile: Optional[bool] = None,
                      apply_background_subtraction: Optional[bool] = None,
                      grayscale: bool = True,
    ...
    Args:
        ...
        config_path: Path to config file. If None, uses config/sensor/gsmini_config.yaml
    """
    ...
    print("=" * 60)
    print("Visualizing Tacniq Gsmini Samples")
    print("=" * 60)
    print(f"\nConfiguration:")
    print(f"  Data root: {data_root}")
    print(f"  Batch size: {batch_size}")
    ...
    if show_background and ("images" in load_modalities):
        bg_path = Path("data/bg_data") / "gsmini.png"
        if bg_path.exists():
            try:
    ...
    if extra_rows == 1:
        ...
        ax_bg.set_title("Background (gsmini.png)", fontsize=10)
        ax_bg.axis('off')
    ...
    for sample_idx in range(actual_num_samples):
        ...
        if has_images:
            ...
            for frame_idx in range(num_frames):
                ...
                # Images are already in RGB format (no conversion needed)
                img_np = img.permute(1, 2, 0).cpu().numpy()  # [H, W, C] in RGB format
    ...
    plt.suptitle(f'Tacniq Gsmini Dataset Visualization ({dataset_split_type})', fontsize=14, y=0.995)
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Test/Visualize Tacniq Gsmini WebDataset dataloader with tar files')
    parser.add_argument('--data-root', type=str, default='data/tacniq_gsmini_tar',
    ...
    parser.add_argument('--output-path', type=str, default='data/tacniq_sample_visualization.png',
    ...
    parser.add_argument('--config-path', type=str, default=None,
                       help='Path to config file (default: config/sensor/gsmini_config.yaml)')
