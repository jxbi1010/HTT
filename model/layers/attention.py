...
from model.layers.mlp import Mlp as MLP
...
class CrossAttention(nn.Module):
    ...
    def forward(self, q: Tensor, x: Tensor):
        ...
        if self.use_sdpa:
            # Use new SDPA kernel API (replaces deprecated torch.backends.cuda.sdp_kernel)
            try:
                ...
                # Fallback for older PyTorch versions or if new API not available
                q = F.scaled_dot_product_attention(q, k, v, dropout_p=self.attn_drop.p)
        else:
