...
2. **[Finetuning onto your own sensor](#finetuning-onto-your-own-sensor)** with
   `finetune_mae.py` — adapt the released backbone to a new sensor from a
...
exact recipe. `run_pretrain.py` runs MAE-only pretraining for a single
modality with the same configs.

**Downstream tasks.** `run_probe.py` evaluates a pretrained checkpoint on
...
```bash
python run_probe.py --task force --modality xela \
...
**Baselines.** `run_spl.py --task_type {force,sliding,classification}
...
`load_baselines.py` runs SITR / T3 through the identical probe pipeline —
